...
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE,fig.width=6, fig.height=4, dpi=90)
library(tidyverse)
...
- Données d'utilisation de Strava : 
  - Intensité d'utilisation de Strava (Facebook Intensity Scale > Strava Intensity Scale ^8^ ^9^)
  - Utilisation des fonctions de sociabilité en ligne (Online Sociability Functions ^9^)
  - Exposition des informations personnelles (Facebook Scale ^9^ > Strava Scale)
  - Compléments
...
- Mesure de l'auto-présentation sur Strava : contrôle de l'image ou authenticité (Self-presentation and Upward Social Comparison Inclination Scale ^10^ ^17^)
- Mesure du stress perçu par l'utilisation de l'application (Perceived Stress Scale ^11^)
- Mesure de la motivation à courir (Behavioural Regulation In Exercise Questionnaire ^12^
)


```{r import donnees}
data<- read_excel("Reponses.xlsx", sheet="Traitement")
```
...
### Fonction générique pour calcul de corrélations
compute_correlations <- function(var, ref_var, data) {
  # Calcul des corrélations Homme et Femme (non affichées)
  corr_H <- round(cor(data %>% filter(Sexe == "1") %>% pull(var), 
                      data %>% filter(Sexe == "1") %>% pull(ref_var), 
                      use = "complete.obs"), 2)
  
  corr_F <- round(cor(data %>% filter(Sexe == "2") %>% pull(var), 
                      data %>% filter(Sexe == "2") %>% pull(ref_var), 
                      use = "complete.obs"), 2)

  # Calcul de la corrélation totale
  corr_total <- round(cor(data[[var]], data[[ref_var]], use = "complete.obs"), 2)

  # Création du tibble avec la corrélation totale, nommée dynamiquement (ex. "Corr_Age")
  result <- tibble(
    Variable = var,
    !!paste0("Corr_", ref_var) := corr_total
  )
...
```{r graphique sexe}
### Calcul et affichage Khi² conformité
...
### Calcul Khi² conformité et affichage
...
```{r fonction pour graphique correlation age}
# Fonction pour générer les graphiques
plot_correlation_age <- function(var, data) {
  # Calcul des coefficients de corrélation
  total_corr <- round(cor(data[[var]], data$Age, use = "complete.obs"), 2)
  corr_h <- round(cor(filter(data, Sexe == "1")[[var]], filter(data, Sexe == "1")$Age, use = "complete.obs"), 2)
  corr_f <- round(cor(filter(data, Sexe == "2")[[var]], filter(data, Sexe == "2")$Age, use = "complete.obs"), 2)
  
  # Préparation des libellés pour la légende
  labels_linetype <- c(
    "Général" = sprintf("Général (R = %.2f)", total_corr),
    "Homme"   = sprintf("Homme (R = %.2f)", corr_h),
    "Femme"   = sprintf("Femme (R = %.2f)", corr_f)
  )
  
  # Création du graphique
  ggplot(data, aes(x = Age, y = .data[[var]],
                   color = factor(Sexe, levels = c(1, 2), labels = c("Homme", "Femme")))) +
    geom_point() +
    geom_smooth(aes(linetype = "Général"), method = "lm", se = FALSE, color = "black") +
    geom_smooth(data = filter(data, Sexe == "1"), aes(linetype = "Homme"), method = "lm", se = FALSE, color = "#FC4C02") +
    geom_smooth(data = filter(data, Sexe == "2"), aes(linetype = "Femme"), method = "lm", se = FALSE, color = "#4F4F4F") +
    scale_color_manual(values = c("Homme" = "#FC4C02", "Femme" = "#4F4F4F")) +
    scale_linetype_manual(name = "Coefficient de corrélation",
                          values = c("Général" = 1, "Homme" = 2, "Femme" = 3),
                          labels = labels_linetype) +
    labs(title = paste("Corrélation entre", variable_labels_strava[[var]], "et l'âge"),
         x = "Âge (en années)",
         y = variable_labels_strava[[var]],
         color = "Sexe") +
    theme_minimal() +
    guides(color = guide_legend(title = "Sexe"))
}
# Liste des variables à tracer
variables_plots_age <- c("SCORE_SIS", "SCORE_OSF", "SCORE_EXPO", "Club_Strava", 
                         "Kudos_reçus", "Fréquence_publication", "Fréquence_consultation", 
                         "Nb_abonnés", "Nb_abonnements", "Ancienneté_Strava")

# Génération automatique des graphiques
plots_age <- map(variables_plots_age, ~ plot_correlation_age(.x, data))
```
...
table_typeabonnement_age <- analyse_t_test("Age", "Type_abonnement", data, 
                                            title_group = "type abonnement", 
                                            title_score = "Age", 
...
variables_strava <- c("SCORE_SIS", "SCORE_OSF", "SCORE_EXPO", "Club_Strava", 
                      "Kudos_reçus", "Fréquence_publication", "Fréquence_consultation", "Nb_abonnés", "Nb_abonnements", "Ancienneté_Strava")
...
resultats_typeabonnement_sexe <- compute_chi2_cramer("Type_abonnement", "Sexe", data)
table_typeabonnement_sexe <- resultats_typeabonnement_sexe$stats  # Résumé Khi²
...
table_comparaison_typeabonnement_sexe <- data.frame(
  Type_abonnement = rep(rownames(resultats_typeabonnement_sexe$observed), times = ncol(resultats_typeabonnement_sexe$observed)),
...
ggplot(heatmap_categorieabonnement_sexe, aes(x = Sexe, y = Type_abonnement, fill = (n/sum(n))*100)) +
...
# Application de la fonction générique pour Type_abonnement et CSP
resultats_typeabonnement_categorie <- compute_chi2_cramer("Type_abonnement", "Catégorie", data)
...
table_comparaison_typeabonnement_categorie <- data.frame(
  Type_abonnement = rep(rownames(resultats_typeabonnement_categorie$observed), times = ncol(resultats_typeabonnement_categorie$observed)),
...
table_comparaison_typeabonnement_categorie$Type_abonnement <- factor(
  table_comparaison_typeabonnement_categorie$Type_abonnement, 
  levels = c("1", "2"), 
  labels = c("Gratuit", "Payant"))
...
heatmap_categorieabonnement_categorie <- data %>%
    filter(!is.na(Type_abonnement), !is.na(Catégorie)) %>%
    group_by(Type_abonnement, Catégorie) %>%
    summarise(n = n(), .groups = "drop")
...
ggplot(heatmap_categorieabonnement_categorie, aes(x = Catégorie, y = Type_abonnement, fill = (n/sum(n))*100)) +
    geom_tile(color = "#eff4f9") +
    scale_fill_gradient(low = "#eff4f9", high = "#FC4C02") +
    geom_text(aes(label = sprintf("%.1f%%", (n / sum(n)) * 100)), color = "black")+
    labs(title = "",
...
# Fonction pour calculer toutes les corrélations et formater en tableau
correlations_sportives <- expand.grid(Strava = variables_strava, Sportive = variables_sportives) %>%
  as_tibble() %>%
  mutate(Correlation = map2_dbl(Strava, Sportive, ~ compute_correlations(.x, .y, data) %>% pull(2)))

# Ajout des labels des variables
correlations_sportives <- correlations_sportives %>%
  mutate(Strava = variable_labels_strava[Strava])

correlations_sportives <- correlations_sportives %>%
  mutate(Sportive = variables_labels_sportives[Sportive])

# Transformation en format large
correlations_sportives_strava <- correlations_sportives %>%
  pivot_wider(names_from = Sportive, values_from = Correlation)


# Mise en forme conditionnelle
correlations_sportives_strava <- correlations_sportives_strava %>%
  mutate(
    Compétitions = case_when(
      as.numeric(abs(Compétitions)) > 0.3 ~ paste0('<span style="color:green;">', Compétitions, '</span>'),
      as.numeric(abs(Compétitions)) > 0.2 ~ paste0('<span style="color:#FC4C02;">', Compétitions, '</span>'),
      as.numeric(abs(Compétitions)) > 0.1 ~ paste0('<span style="color:red;">', Compétitions, '</span>'),
      TRUE ~ as.character(Compétitions)
    )
  )

correlations_sportives_strava <- correlations_sportives_strava %>%
  mutate(
    `Nombre de kilomètres par semaine` = case_when(
      as.numeric(abs(`Nombre de kilomètres par semaine`)) > 0.3 ~ paste0('<span style="color:green;">', `Nombre de kilomètres par semaine`, '</span>'),
      as.numeric(abs(`Nombre de kilomètres par semaine`)) > 0.2 ~ paste0('<span style="color:#FC4C02;">', `Nombre de kilomètres par semaine`, '</span>'),
      as.numeric(abs(`Nombre de kilomètres par semaine`)) > 0.1 ~ paste0('<span style="color:red;">', `Nombre de kilomètres par semaine`, '</span>'),
      TRUE ~ as.character(`Nombre de kilomètres par semaine`)
    )
  )

correlations_sportives_strava <- correlations_sportives_strava %>%
  mutate(
    `Nombre d'entraînements par semaine` = case_when(
      as.numeric(abs(`Nombre d'entraînements par semaine`)) > 0.3 ~ paste0('<span style="color:green;">', `Nombre d'entraînements par semaine`, '</span>'),
      as.numeric(abs(`Nombre d'entraînements par semaine`)) > 0.2 ~ paste0('<span style="color:#FC4C02;">', `Nombre d'entraînements par semaine`, '</span>'),
      as.numeric(abs(`Nombre d'entraînements par semaine`)) > 0.1 ~ paste0('<span style="color:red;">', `Nombre d'entraînements par semaine`, '</span>'),
      TRUE ~ as.character(`Nombre d'entraînements par semaine`)
    )
  )

# Affichage en HTML
kable(correlations_sportives_strava, format = "html", escape = FALSE)
```

#### Visualisation graphique{.tabset}

```{r visualisation graphique strava pratique}
# Fonction pour générer les graphiques pour kilomètres
plot_correlation_km <- function(var, data) {
  # Calcul des coefficients de corrélation
  total_corr <- round(cor(data[[var]], data$Kilomètres, use = "complete.obs"), 2)
  corr_h <- round(cor(filter(data, Sexe == "1")[[var]], filter(data, Sexe == "1")$Kilomètres, use = "complete.obs"), 2)
  corr_f <- round(cor(filter(data, Sexe == "2")[[var]], filter(data, Sexe == "2")$Kilomètres, use = "complete.obs"), 2)
  ...
  # Création du graphique
  ggplot(data, aes(x = Kilomètres, y = .data[[var]],
                   color = factor(Sexe, levels = c(1, 2), labels = c("Homme", "Femme")))) +
    geom_point() +
    geom_smooth(aes(linetype = "Général"), method = "lm", se = FALSE, color = "black") +
    geom_smooth(data = filter(data, Sexe == "1"), aes(linetype = "Homme"), method = "lm", se = FALSE, color = "#FC4C02") +
    ...
    scale_linetype_manual(name = "Coefficient de corrélation",
                          values = c("Général" = 1, "Homme" = 2, "Femme" = 3),
                          labels = labels_linetype) +
    labs(title = paste("Corrélation entre", variable_labels_strava[[var]], "et le nombre de kilomètres par semaine"),
         x = "Kilomètres par semaine",
         y = variable_labels_strava[[var]],
         color = "Sexe") +
    theme_minimal() +
    guides(color = guide_legend(title = "Sexe"))
...
# Liste des variables à tracer
variables_plots_km <- c("SCORE_SIS", "SCORE_OSF", "SCORE_EXPO", "Club_Strava", 
                         "Kudos_reçus", "Fréquence_publication", "Fréquence_consultation", 
                         "Nb_abonnés", "Nb_abonnements", "Ancienneté_Strava")

# Génération automatique des graphiques
plots_strava_km <- map(variables_plots_km, ~ plot_correlation_km(.x, data))

# Fonction pour générer les graphiques pour freq pratique
plot_correlation_freqpratique <- function(var, data) {
  # Calcul des coefficients de corrélation
  total_corr <- round(cor(data[[var]], data$Frequence_pratique, use = "complete.obs"), 2)
  corr_h <- round(cor(filter(data, Sexe == "1")[[var]], filter(data, Sexe == "1")$Frequence_pratique, use = "complete.obs"), 2)
  corr_f <- round(cor(filter(data, Sexe == "2")[[var]], filter(data, Sexe == "2")$Frequence_pratique, use = "complete.obs"), 2)
  ...
  # Création du graphique
  ggplot(data, aes(x = Frequence_pratique, y = .data[[var]],
                   color = factor(Sexe, levels = c(1, 2), labels = c("Homme", "Femme")))) +
    geom_point() +
    geom_smooth(aes(linetype = "Général"), method = "lm", se = FALSE, color = "black") +
    geom_smooth(data = filter(data, Sexe == "1"), aes(linetype = "Homme"), method = "lm", se = FALSE, color = "#FC4C02") +
    ...
    scale_linetype_manual(name = "Coefficient de corrélation",
                          values = c("Général" = 1, "Homme" = 2, "Femme" = 3),
                          labels = labels_linetype) +
    labs(title = paste("Corrélation entre", variable_labels_strava[[var]], "et le nombre d'entraînements par semaine"),
...
# Liste des variables à tracer
variables_plots_freqpratique <- c("SCORE_SIS", "SCORE_OSF", "SCORE_EXPO", "Club_Strava", 
                         "Kudos_reçus", "Fréquence_publication", "Fréquence_consultation", 
                         "Nb_abonnés", "Nb_abonnements", "Ancienneté_Strava")

# Génération automatique des graphiques
plots_strava_freqpratique <- map(variables_plots_freqpratique, ~ plot_correlation_freqpratique(.x, data))

# Fonction pour générer les graphiques pour compétitions
plot_correlation_compet <- function(var, data) {
  # Calcul des coefficients de corrélation
  total_corr <- round(cor(data[[var]], data$Compétitions, use = "complete.obs"), 2)
  corr_h <- round(cor(filter(data, Sexe == "1")[[var]], filter(data, Sexe == "1")$Compétitions, use = "complete.obs"), 2)
  corr_f <- round(cor(filter(data, Sexe == "2")[[var]], filter(data, Sexe == "2")$Compétitions, use = "complete.obs"), 2)
  ...
  # Création du graphique
  ggplot(data, aes(x = Compétitions, y = .data[[var]],
                   color = factor(Sexe, levels = c(1, 2), labels = c("Homme", "Femme")))) +
    geom_point() +
    geom_smooth(aes(linetype = "Général"), method = "lm", se = FALSE, color = "black") +
    geom_smooth(data = filter(data, Sexe == "1"), aes(linetype = "Homme"), method = "lm", se = FALSE, color = "#FC4C02") +
    ...
    scale_linetype_manual(name = "Coefficient de corrélation",
                          values = c("Général" = 1, "Homme" = 2, "Femme" = 3),
                          labels = labels_linetype) +
    labs(title = paste("Corrélation entre", variable_labels_strava[[var]], "et le nombre de compétitions par an"),
         x = "Compétitions par an",
         y = variable_labels_strava[[var]],
         color = "Sexe") +
    theme_minimal() +
    guides(color = guide_legend(title = "Sexe"))
...
# Liste des variables à tracer
variables_plots_compet <- c("SCORE_SIS", "SCORE_OSF", "SCORE_EXPO", "Club_Strava", 
                         "Kudos_reçus", "Fréquence_publication", "Fréquence_consultation", 
                         "Nb_abonnés", "Nb_abonnements", "Ancienneté_Strava")

# Génération automatique des graphiques
plots_strava_compet <- map(variables_plots_compet, ~ plot_correlation_compet(.x, data))
...
ggplot(heatmap_categorieabonnement_niveau, aes(x = Niveau, y = Type_abonnement, fill = (n/sum(n))*100)) +
    geom_tile(color = "#eff4f9") +
    scale_fill_gradient(low = "#eff4f9", high = "#FC4C02") +
...
ggplot(heatmap_categorieabonnement_typecoureur, aes(x = Type_coureur, y = Type_abonnement, fill = (n/sum(n))*100)) +
    geom_tile(color = "#eff4f9") +
    scale_fill_gradient(low = "#eff4f9", high = "#FC4C02") +
...
ggplot(heatmap_categorieabonnement_objectif, aes(x = Objectif_KM, y = Type_abonnement, fill = (n/sum(n))*100)) +
    geom_tile(color = "#eff4f9") +
    scale_fill_gradient(low = "#eff4f9", high = "#FC4C02") +
...
### SIS – SPAUSCIS
cor_SIS_SPAUSCIS <- round(cor(data$SCORE_SIS, data$SCORE_SPAUSCIS, use="complete.obs"),2)
cor_SIS_SPAUSCISh <- round(cor(subset(data, Sexe == "1")$SCORE_SIS, subset(data, Sexe == "1")$SCORE_SPAUSCIS, use="complete.obs"),2)
cor_SIS_SPAUSCISf <- round(cor(subset(data, Sexe == "2")$SCORE_SIS, subset(data, Sexe == "2")$SCORE_SPAUSCIS, use="complete.obs"),2)
...
ggplot(data, aes(x = SCORE_SIS, y = SCORE_SPAUSCIS, 
                 color = factor(Sexe, levels = c(1, 2), labels = c("Homme", "Femme")))) +
    geom_point() +  
...
cor_SIS_PSS_Stress<- round(cor(data$SCORE_SIS, data$SCORE_PSS_Stress, use = "complete.obs"), 2)

cor_SIS_PSS_Stressh <- round(cor(subset(data, Sexe == "1")$SCORE_SIS, 
                 subset(data, Sexe == "1")$SCORE_PSS_Stress, use = "complete.obs"), 2)
cor_SIS_PSS_Stressf<- round(cor(subset(data, Sexe == "2")$SCORE_SIS, 
                 subset(data, Sexe == "2")$SCORE_PSS_Stress, use = "complete.obs"), 2)

#### SIS - PSS Adaptation

cor_SIS_PSS_Adaptation<- round(cor(data$SCORE_SIS, data$SCORE_PSS_Adaptation, use = "complete.obs"), 2)

cor_SIS_PSS_Adaptationh <- round(cor(subset(data, Sexe == "1")$SCORE_SIS, 
                 subset(data, Sexe == "1")$SCORE_PSS_Adaptation, use = "complete.obs"), 2)
cor_SIS_PSS_Adaptationf<- round(cor(subset(data, Sexe == "2")$SCORE_SIS, 
...
table_PSS_Correlations <- data.frame(
  Motivation = c("SCORE SIS ", "SCORE OSF ", "SCORE EXPO ", "Clubs Strava ", "Kudos reçus ", "Fréquence publication ", "Fréquence consultation ", "Nombre d'abonnés ", "Nombre d'abonnements ", "Ancienneté Strava "),
  PSS_Stress = c(cor_SIS_PSS_Stress, cor_OSF_PSS_Stress, cor_EXPO_PSS_Stress, cor_Club_Strava_PSS_Stress, cor_Kudos_recus_PSS_Stress, cor_freq_publi_PSS_Stress, cor_freq_cons_PSS_Stress, cor_nbabonnes_PSS_Stress, cor_nbabonnements_PSS_Stress, cor_anciennetestrava_PSS_Stress),
  PSS_Adaptation = c(cor_SIS_PSS_Adaptation, cor_OSF_PSS_Adaptation, cor_EXPO_PSS_Adaptation, cor_Club_Strava_PSS_Adaptation, cor_Kudos_recus_PSS_Adaptation, cor_freq_publi_PSS_Adaptation, cor_freq_cons_PSS_Adaptation, cor_nbabonnes_PSS_Adaptation, cor_nbabonnements_PSS_Adaptation, cor_anciennetestrava_PSS_Adaptation))
...
ggplot(data, aes(x = SCORE_SIS , y = SCORE_PSS_Adaptation, 
                 color = factor(Sexe, levels = c(1, 2), labels = c("Homme", "Femme")))) +
    geom_point() +  
...
scale_color_manual(values = c("Homme" = "#FC4C02", "Femme" = "#4F4F4F"))+
      labs(title = "SCORE PSS Adaptation en fonction du SCORE EXPO",
         x = "SCORE Exposition",
         y = "SCORE PSS Adaptation",
         color = "Sexe",
         linetype = "Coefficient de corrélation") +
...
cor_SIS_intrinseque<- round(cor(data$SCORE_SIS, data$SCORE_BREQ_Intrinseque, use = "complete.obs"), 2)

cor_SIS_intrinsequeh <- round(cor(subset(data, Sexe == "1")$SCORE_SIS, 
                 subset(data, Sexe == "1")$SCORE_BREQ_Intrinseque, use = "complete.obs"), 2)
cor_SIS_intrinsequef <- round(cor(subset(data, Sexe == "2")$SCORE_SIS, 
                 subset(data, Sexe == "2")$SCORE_BREQ_Intrinseque, use = "complete.obs"), 2)

cor_SIS_integree<- round(cor(data$SCORE_SIS, data$SCORE_BREQ_Integree, use = "complete.obs"), 2)
cor_SIS_integreeh <- round(cor(subset(data, Sexe == "1")$SCORE_SIS, 
                 subset(data, Sexe == "1")$SCORE_BREQ_Integree, use = "complete.obs"), 2)
cor_SIS_integreef <- round(cor(subset(data, Sexe == "2")$SCORE_SIS, 
                 subset(data, Sexe == "2")$SCORE_BREQ_Integree, use = "complete.obs"), 2)

cor_SIS_identifiee<- round(cor(data$SCORE_SIS, data$SCORE_BREQ_Identifiee, use = "complete.obs"), 2)
cor_SIS_identifieeh <- round(cor(subset(data, Sexe == "1")$SCORE_SIS, 
                 subset(data, Sexe == "1")$SCORE_BREQ_Identifiee, use = "complete.obs"), 2)
cor_SIS_identifieef <- round(cor(subset(data, Sexe == "2")$SCORE_SIS, 
                 subset(data, Sexe == "2")$SCORE_BREQ_Identifiee, use = "complete.obs"), 2)

cor_SIS_introjectee<- round(cor(data$SCORE_SIS, data$SCORE_BREQ_Introjectee, use = "complete.obs"), 2)
cor_SIS_introjecteeh <- round(cor(subset(data, Sexe == "1")$SCORE_SIS, 
                 subset(data, Sexe == "1")$SCORE_BREQ_Introjectee, use = "complete.obs"), 2)
cor_SIS_introjecteef <- round(cor(subset(data, Sexe == "2")$SCORE_SIS, 
                 subset(data, Sexe == "2")$SCORE_BREQ_Introjectee, use = "complete.obs"), 2)

cor_SIS_externe<- round(cor(data$SCORE_SIS, data$SCORE_BREQ_Externe, use = "complete.obs"), 2)
cor_SIS_externeh <- round(cor(subset(data, Sexe == "1")$SCORE_SIS, 
                 subset(data, Sexe == "1")$SCORE_BREQ_Externe, use = "complete.obs"), 2)
cor_SIS_externef <- round(cor(subset(data, Sexe == "2")$SCORE_SIS, 
                 subset(data, Sexe == "2")$SCORE_BREQ_Externe, use = "complete.obs"), 2)

cor_SIS_amotivation<- round(cor(data$SCORE_SIS, data$SCORE_BREQA, use = "complete.obs"), 2)
cor_SIS_amotivationh <- round(cor(subset(data, Sexe == "1")$SCORE_SIS, 
                 subset(data, Sexe == "1")$SCORE_BREQA, use = "complete.obs"), 2)
cor_SIS_amotivationf <- round(cor(subset(data, Sexe == "2")$SCORE_SIS, 
                 subset(data, Sexe == "2")$SCORE_BREQA, use = "complete.obs"), 2)

cor_SIS_rai<- round(cor(data$SCORE_SIS, data$SCORE_RAI, use = "complete.obs"), 2)
cor_SIS_raih <- round(cor(subset(data, Sexe == "1")$SCORE_SIS, 
                 subset(data, Sexe == "1")$SCORE_RAI, use = "complete.obs"), 2)
cor_SIS_raif <- round(cor(subset(data, Sexe == "2")$SCORE_SIS, 
...
table_Motivation_Correlations <- data.frame(
  Motivation = c("SCORE SIS ", "SCORE OSF ", "SCORE EXPO ", "Clubs Strava ", "Kudos reçus ", "Fréquence publication ", "Fréquence consultation ", "Nombre d'abonnés ", "Nombre d'abonnements ", "Ancienneté Strava "),
  Intrinsèque = c(cor_SIS_intrinseque, cor_OSF_intrinseque, cor_EXPO_intrinseque, cor_Club_Strava_intrinseque, cor_Kudos_recus_intrinseque, cor_freq_publi_intrinseque, cor_freq_cons_intrinseque, cor_nbabonnes_intrinseque, cor_nbabonnements_intrinseque, cor_anciennetestrava_intrinseque),
  Integrée = c(cor_SIS_integree, cor_OSF_integree, cor_EXPO_integree, cor_Club_Strava_integree, cor_Kudos_recus_integree, cor_freq_publi_integree, cor_freq_cons_integree, cor_nbabonnes_integree, cor_nbabonnements_integree, cor_anciennetestrava_integree),
  Identifiée = c(cor_SIS_identifiee, cor_OSF_identifiee, cor_EXPO_identifiee, cor_Club_Strava_identifiee, cor_Kudos_recus_identifiee, cor_freq_publi_identifiee, cor_freq_cons_identifiee, cor_nbabonnes_identifiee, cor_nbabonnements_identifiee, cor_anciennetestrava_identifiee),
  Introjectée = c(cor_SIS_introjectee, cor_OSF_introjectee, cor_EXPO_introjectee, cor_Club_Strava_introjectee, cor_Kudos_recus_introjectee, cor_freq_publi_introjectee, cor_freq_cons_introjectee, cor_nbabonnes_introjectee, cor_nbabonnements_introjectee, cor_anciennetestrava_introjectee),
  Externe = c(cor_SIS_externe, cor_OSF_externe, cor_EXPO_externe, cor_Club_Strava_externe, cor_Kudos_recus_externe, cor_freq_publi_externe, cor_freq_cons_externe, cor_nbabonnes_externe, cor_nbabonnements_externe, cor_anciennetestrava_externe),
  Amotivation = c(cor_SIS_amotivation, cor_OSF_amotivation, cor_EXPO_amotivation, cor_Club_Strava_amotivation, cor_Kudos_recus_amotivation, cor_freq_publi_amotivation, cor_freq_cons_amotivation, cor_nbabonnes_amotivation, cor_nbabonnements_amotivation, cor_anciennetestrava_amotivation),
  RAI =  c(cor_SIS_rai, cor_OSF_rai, cor_EXPO_rai, cor_Club_Strava_rai, cor_Kudos_recus_rai, cor_freq_publi_rai, cor_freq_cons_rai, cor_nbabonnes_rai, cor_nbabonnements_rai, cor_anciennetestrava_rai)
)

table_Motivation_Correlations$Intrinsèque <- ifelse(
  table_Motivation_Correlations$Intrinsèque > 0.3, 
  paste0('<span style="color:green;">', table_Motivation_Correlations$Intrinsèque, '</span>'),
  ifelse(
    table_Motivation_Correlations$Intrinsèque > 0.2, 
    paste0('<span style="color:#FC4C02;">', table_Motivation_Correlations$Intrinsèque, '</span>'),
    ifelse(
      table_Motivation_Correlations$Intrinsèque > 0.1, 
      paste0('<span style="color:red;">', table_Motivation_Correlations$Intrinsèque, '</span>'),
      table_Motivation_Correlations$Intrinsèque
    )
  )
)

table_Motivation_Correlations$Integrée <- ifelse(
  table_Motivation_Correlations$Integrée > 0.3, 
  paste0('<span style="color:green;">', table_Motivation_Correlations$Integrée, '</span>'),
  ifelse(
    table_Motivation_Correlations$Integrée > 0.2, 
    paste0('<span style="color:#FC4C02;">', table_Motivation_Correlations$Integrée, '</span>'),
    ifelse(
      table_Motivation_Correlations$Integrée > 0.1, 
      paste0('<span style="color:red;">', table_Motivation_Correlations$Integrée, '</span>'),
      table_Motivation_Correlations$Integrée
    )
  )
)

table_Motivation_Correlations$Identifiée <- ifelse(
  table_Motivation_Correlations$Identifiée > 0.3, 
  paste0('<span style="color:green;">', table_Motivation_Correlations$Identifiée, '</span>'),
  ifelse(
    table_Motivation_Correlations$Identifiée > 0.2, 
    paste0('<span style="color:#FC4C02;">', table_Motivation_Correlations$Identifiée, '</span>'),
    ifelse(
      table_Motivation_Correlations$Identifiée > 0.1, 
      paste0('<span style="color:red;">', table_Motivation_Correlations$Identifiée, '</span>'),
      table_Motivation_Correlations$Identifiée
    )
  )
)

table_Motivation_Correlations$Introjectée <- ifelse(
  table_Motivation_Correlations$Introjectée > 0.3, 
  paste0('<span style="color:green;">', table_Motivation_Correlations$Introjectée, '</span>'),
  ifelse(
    table_Motivation_Correlations$Introjectée > 0.2, 
    paste0('<span style="color:#FC4C02;">', table_Motivation_Correlations$Introjectée, '</span>'),
    ifelse(
      table_Motivation_Correlations$Introjectée > 0.1, 
      paste0('<span style="color:red;">', table_Motivation_Correlations$Introjectée, '</span>'),
      table_Motivation_Correlations$Introjectée
    )
  )
)
...
ggplot(data, aes(x = SCORE_SIS , y = SCORE_BREQ_Introjectee, 
                 color = factor(Sexe, levels = c(1, 2), labels = c("Homme", "Femme")))) +
    geom_point() +  
    geom_smooth(method = "lm", se = FALSE, aes(linetype = sprintf("Général (R = %.2f)", cor_SIS_introjectee)), color = "black") +  
    geom_smooth(data = subset(data, Sexe == "1"), method = "lm", se = FALSE, 
                aes(linetype = sprintf("Homme (R = %.2f)", cor_SIS_introjecteeh)), color = "#FC4C02") +  
    geom_smooth(data = subset(data, Sexe == "2"), method = "lm", se = FALSE, 
                aes(linetype = sprintf("Femme (R = %.2f)", cor_SIS_introjecteef)), color = "#4F4F4F") +
  scale_color_manual(values = c("Homme" = "#FC4C02", "Femme" = "#4F4F4F"))+
...
ggplot(data, aes(x = Kudos_reçus , y = SCORE_BREQ_Integree, 
    ...
    geom_smooth(data = subset(data, Sexe == "1"), method = "lm", se = FALSE, 
                aes(linetype = sprintf("Homme (R = %.2f)",cor_Kudos_recus_integreeh)), color = "#FC4C02") +  
    geom_smooth(data = subset(data, Sexe == "2"), method = "lm", se = FALSE, 
                aes(linetype = sprintf("Femme (R = %.2f)", cor_Kudos_recus_integreef)), color = "#4F4F4F") +
  scale_color_manual(values = c("Homme" = "#FC4C02", "Femme" = "#4F4F4F"))+
...
ggplot(data, aes(x = Fréquence_publication , y = SCORE_BREQ_Intrinseque, 
    ...
    geom_smooth(data = subset(data, Sexe == "1"), method = "lm", se = FALSE, 
                aes(linetype = sprintf("Homme (R = %.2f)",cor_freq_publi_intrinsequeh)), color = "#FC4C02") +  
    geom_smooth(data = subset(data, Sexe == "2"), method = "lm", se = FALSE, 
                aes(linetype = sprintf("Femme (R = %.2f)", cor_freq_publi_intrinsequef)), color = "#4F4F4F") +
  scale_color_manual(values = c("Homme" = "#FC4C02", "Femme" = "#4F4F4F"))+
...
ggplot(data, aes(x = Fréquence_publication , y = SCORE_BREQ_Integree, 
    ...
    geom_smooth(data = subset(data, Sexe == "1"), method = "lm", se = FALSE, 
                aes(linetype = sprintf("Homme (R = %.2f)",cor_freq_publi_integreeh)), color = "#FC4C02") +  
    geom_smooth(data = subset(data, Sexe == "2"), method = "lm", se = FALSE, 
                aes(linetype = sprintf("Femme (R = %.2f)", cor_freq_publi_integreef)), color = "#4F4F4F") +
  scale_color_manual(values = c("Homme" = "#FC4C02", "Femme" = "#4F4F4F"))+
...
ggplot(data, aes(x = Fréquence_publication , y = SCORE_BREQ_Identifiee, 
    ...
    geom_smooth(data = subset(data, Sexe == "1"), method = "lm", se = FALSE, 
                aes(linetype = sprintf("Homme (R = %.2f)",cor_freq_publi_identifieeh)), color = "#FC4C02") +  
    geom_smooth(data = subset(data, Sexe == "2"), method = "lm", se = FALSE, 
                aes(linetype = sprintf("Femme (R = %.2f)", cor_freq_publi_identifieef)), color = "#4F4F4F") +
  scale_color_manual(values = c("Homme" = "#FC4C02", "Femme" = "#4F4F4F"))+
...
ggplot(data, aes(x = Nb_abonnements , y = SCORE_BREQ_Integree, 
    ...
    geom_smooth(data = subset(data, Sexe == "1"), method = "lm", se = FALSE, 
                aes(linetype = sprintf("Homme (R = %.2f)",cor_nbabonnements_integreeh)), color = "#FC4C02") +  
    geom_smooth(data = subset(data, Sexe == "2"), method = "lm", se = FALSE, 
                aes(linetype = sprintf("Femme (R = %.2f)", cor_nbabonnements_integreef)), color = "#4F4F4F") +
   scale_color_manual(values = c("Homme" = "#FC4C02", "Femme" = "#4F4F4F"))+
...
t_typeabonnement_intrinseque <- t.test(as.numeric(SCORE_BREQ_Intrinseque) ~ as.factor(Type_abonnement), data = data, var.equal = TRUE)

t_typeabonnement_introjectee <- t.test(as.numeric(SCORE_BREQ_Introjectee) ~ as.factor(Type_abonnement), data = data, var.equal = TRUE)

t_typeabonnement_identifiee <- t.test(as.numeric(SCORE_BREQ_Identifiee) ~ as.factor(Type_abonnement), data = data, var.equal = TRUE)

t_typeabonnement_integree <- t.test(as.numeric(SCORE_BREQ_Integree) ~ as.factor(Type_abonnement), data = data, var.equal = TRUE)

t_typeabonnement_externe <- t.test(as.numeric(SCORE_BREQ_Externe) ~ as.factor(Type_abonnement), data = data, var.equal = TRUE)

t_typeabonnement_amotivation <- t.test(as.numeric(SCORE_BREQA) ~ as.factor(Type_abonnement), data = data, var.equal = TRUE)

t_typeabonnement_rai <- t.test(as.numeric(SCORE_RAI) ~ as.factor(Type_abonnement), data = data, var.equal = TRUE)
...
#### Corrélations
```{r correlation pratique BREQ}
### Freq entrainement – BREQ
cor_freqentrainement_Intrinseque <- round(cor(data$Frequence_pratique, data$SCORE_BREQ_Intrinseque, use="complete.obs"),2)
cor_freqentrainement_Integree <- round(cor(data$Frequence_pratique, data$SCORE_BREQ_Integree, use="complete.obs"),2)
cor_freqentrainement_Introjectee <- round(cor(data$Frequence_pratique, data$SCORE_BREQ_Introjectee, use="complete.obs"),2)
cor_freqentrainement_Identifiee <- round(cor(data$Frequence_pratique, data$SCORE_BREQ_Identifiee, use="complete.obs"),2)
cor_freqentrainement_Externe <- round(cor(data$Frequence_pratique, data$SCORE_BREQ_Externe, use="complete.obs"),2)
cor_freqentrainement_Amotivation <- round(cor(data$Frequence_pratique, data$SCORE_BREQA, use="complete.obs"),2)
cor_freqentrainement_rai <- round(cor(data$Frequence_pratique, data$SCORE_RAI, use="complete.obs"),2)

#### Volume – SPAUSCIS
cor_km_Intrinseque <- round(cor(data$Kilomètres, data$SCORE_BREQ_Intrinseque, use="complete.obs"),2)
cor_km_Integree <- round(cor(data$Kilomètres, data$SCORE_BREQ_Integree, use="complete.obs"),2)
cor_km_Identifiee <- round(cor(data$Kilomètres, data$SCORE_BREQ_Identifiee, use="complete.obs"),2)
cor_km_Introjectee <- round(cor(data$Kilomètres, data$SCORE_BREQ_Introjectee, use="complete.obs"),2)
cor_km_Externe <- round(cor(data$Kilomètres, data$SCORE_BREQ_Externe, use="complete.obs"),2)
cor_km_Amotivation <- round(cor(data$Kilomètres, data$SCORE_BREQA, use="complete.obs"),2)
cor_km_rai <- round(cor(data$Kilomètres, data$SCORE_RAI, use="complete.obs"),2)

#### Compétitions – SPAUSCIS
cor_compet_Intrinseque <- round(cor(data$Compétitions, data$SCORE_BREQ_Intrinseque, use="complete.obs"),2)
cor_compet_Introjectee <- round(cor(data$Compétitions, data$SCORE_BREQ_Introjectee, use="complete.obs"),2)
cor_compet_Identifiee<- round(cor(data$Compétitions, data$SCORE_BREQ_Identifiee, use="complete.obs"),2)
cor_compet_Integree<- round(cor(data$Compétitions, data$SCORE_BREQ_Integree, use="complete.obs"),2)
cor_compet_Externe<- round(cor(data$Compétitions, data$SCORE_BREQ_Externe, use="complete.obs"),2)
cor_compet_Amotivation<- round(cor(data$Compétitions, data$SCORE_BREQA, use="complete.obs"),2)
cor_compet_rai<- round(cor(data$Compétitions, data$SCORE_RAI, use="complete.obs"),2)

table_BREQ_pratique_Correlations <- data.frame(
  Variable = c("Nombre d'entraînements par semaine ", "Nombre de kilomètres par semaine ", "Nombre de compétitions par an"),
  Intrinsèque = c(cor_freqentrainement_Intrinseque, cor_km_Intrinseque, cor_compet_Intrinseque),
  Intégrée = c(cor_freqentrainement_Integree, cor_km_Integree, cor_compet_Integree),
  Identifiée = c(cor_freqentrainement_Identifiee, cor_km_Identifiee, cor_compet_Identifiee),
  Introjectée = c(cor_freqentrainement_Introjectee, cor_km_Introjectee, cor_compet_Introjectee),
  Externe = c(cor_freqentrainement_Externe, cor_km_Externe, cor_compet_Externe),
  Amotivation =c(cor_freqentrainement_Amotivation, cor_km_Amotivation, cor_compet_Amotivation),
  RAI = c(cor_freqentrainement_rai, cor_km_rai, cor_compet_rai)
  )

table_BREQ_pratique_Correlations$Intrinsèque <- case_when(
table_BREQ_pratique_Correlations$Intrinsèque > 0.4 ~ 
    paste0('<span style="color:green;">', table_BREQ_pratique_Correlations$Intrinsèque, '</span>'),
  table_BREQ_pratique_Correlations$Intrinsèque > 0.3 ~ 
    paste0('<span style="color:#FC4C02;">',table_BREQ_pratique_Correlations$Intrinsèque, '</span>'),
 table_BREQ_pratique_Correlations$Intrinsèque > 0.2 ~ 
    paste0('<span style="color:red;">', table_BREQ_pratique_Correlations$Intrinsèque, '</span>'),
  TRUE ~ as.character(table_BREQ_pratique_Correlations$Intrinsèque)
)

table_BREQ_pratique_Correlations$Introjectée <- case_when(
table_BREQ_pratique_Correlations$Introjectée > 0.4 ~ 
    paste0('<span style="color:green;">', table_BREQ_pratique_Correlations$Introjectée, '</span>'),
 table_BREQ_pratique_Correlations$Introjectée > 0.3 ~ 
    paste0('<span style="color:#FC4C02;">',table_BREQ_pratique_Correlations$Introjectée, '</span>'),
 table_BREQ_pratique_Correlations$Introjectée > 0.2 ~ 
    paste0('<span style="color:red;">', table_BREQ_pratique_Correlations$Introjectée, '</span>'),
  TRUE ~ as.character(table_BREQ_pratique_Correlations$Introjectée)
)

table_BREQ_pratique_Correlations$Intégrée <- case_when(
table_BREQ_pratique_Correlations$Intégrée > 0.4 ~ 
    paste0('<span style="color:green;">', table_BREQ_pratique_Correlations$Intégrée, '</span>'),
 table_BREQ_pratique_Correlations$Intégrée > 0.3 ~ 
    paste0('<span style="color:#FC4C02;">',table_BREQ_pratique_Correlations$Intégrée, '</span>'),
 table_BREQ_pratique_Correlations$Intégrée > 0.2 ~ 
    paste0('<span style="color:red;">', table_BREQ_pratique_Correlations$Intégrée, '</span>'),
  TRUE ~ as.character(table_BREQ_pratique_Correlations$Intégrée)
)

table_BREQ_pratique_Correlations$Identifiée <- case_when(
table_BREQ_pratique_Correlations$Identifiée > 0.4 ~ 
    paste0('<span style="color:green;">', table_BREQ_pratique_Correlations$Identifiée, '</span>'),
 table_BREQ_pratique_Correlations$Identifiée > 0.3 ~ 
    paste0('<span style="color:#FC4C02;">',table_BREQ_pratique_Correlations$Identifiée, '</span>'),
 table_BREQ_pratique_Correlations$Identifiée > 0.2 ~ 
    paste0('<span style="color:red;">', table_BREQ_pratique_Correlations$Identifiée, '</span>'),
  TRUE ~ as.character(table_BREQ_pratique_Correlations$Identifiée)
)

table_BREQ_pratique_Correlations$Externe <- case_when(
table_BREQ_pratique_Correlations$Externe > 0.4 ~ 
    paste0('<span style="color:green;">', table_BREQ_pratique_Correlations$Externe, '</span>'),
 table_BREQ_pratique_Correlations$Externe > 0.3 ~ 
    paste0('<span style="color:#FC4C02;">',table_BREQ_pratique_Correlations$Externe, '</span>'),
 table_BREQ_pratique_Correlations$Externe > 0.2 ~ 
    paste0('<span style="color:red;">', table_BREQ_pratique_Correlations$Externe, '</span>'),
  TRUE ~ as.character(table_BREQ_pratique_Correlations$Externe)
)

table_BREQ_pratique_Correlations$Amotivation <- case_when(
table_BREQ_pratique_Correlations$Amotivation > 0.4 ~ 
    paste0('<span style="color:green;">', table_BREQ_pratique_Correlations$Amotivation, '</span>'),
 table_BREQ_pratique_Correlations$Amotivation > 0.3 ~ 
    paste0('<span style="color:#FC4C02;">',table_BREQ_pratique_Correlations$Amotivation, '</span>'),
 table_BREQ_pratique_Correlations$Amotivation > 0.2 ~ 
    paste0('<span style="color:red;">', table_BREQ_pratique_Correlations$Amotivation, '</span>'),
  TRUE ~ as.character(table_BREQ_pratique_Correlations$Amotivation)
)

table_BREQ_pratique_Correlations$RAI <- case_when(
table_BREQ_pratique_Correlations$RAI > 0.4 ~ 
    paste0('<span style="color:green;">', table_BREQ_pratique_Correlations$RAI, '</span>'),
 table_BREQ_pratique_Correlations$RAI > 0.3 ~ 
    paste0('<span style="color:#FC4C02;">',table_BREQ_pratique_Correlations$RAI, '</span>'),
 table_BREQ_pratique_Correlations$RAI > 0.2 ~ 
    paste0('<span style="color:red;">', table_BREQ_pratique_Correlations$RAI, '</span>'),
  TRUE ~ as.character(table_BREQ_pratique_Correlations$RAI)
)

kable(table_BREQ_pratique_Correlations, format = "html", escape = FALSE)
